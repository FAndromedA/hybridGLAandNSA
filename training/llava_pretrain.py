...
def init_model(model_config: HybridLlavaConfig):
    ...
    model.text_model = AutoModel.from_pretrained(text_model_name_or_path)
    ...
    if hasattr(model.text_model, "layers"):
        last_two_layers = model.text_model.layers[-2:]
    ...
    for epoch in range(args.epochs):
        ...
        for step, (X, Y, loss_mask, pixel_tensors) in enumerate(train_loader):
            ...
            if accelerator.is_main_process and step % args.log_interval == 0:
                logger.info(f"epoch {epoch} step {step} / {len(train_loader)} loss: {loss:.5f} lr: {lr_scheduler.get_last_lr()[0]:.5f}") 
